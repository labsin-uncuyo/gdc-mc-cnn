...
--   cmd:option('-dataset', 'dataset', 'Path to the dataset folder')
   ...
   cmd:option('-after', 1, 'Test every epoch after this one')
   cmd:option('-make_cache', false)
   cmd:option('-use_cache', false)
   cmd:option('-save_img', true)

   -- Parameters of the matching cost network
   cmd:option('-cl', 2, 'Convolutional Layers number')
   cmd:option('-fm', 32, 'Number of feature maps for convolutional layers')
   cmd:option('-ks', 3, 'Fixed kernel size')
   cmd:option('-cks', '', 'Custom kernel sizes')
   cmd:option('-fcl', 2, 'Fully Connected Layers number')
   cmd:option('-nu', 16, 'Number of units for fully connected layers')
