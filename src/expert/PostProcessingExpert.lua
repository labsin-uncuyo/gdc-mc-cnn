...
local PostProcessingExpert = torch.class('PostProcessingExpert')

function PostProcessingExpert:__init()
   
end

function PostProcessingExpert:process(vols, x_batch, disp_max, params, dataset, sm_terminate, sm_skip , directions)
   ...
   for _, direction in ipairs(directions) do
       ...
       if sm_active and sm_skip ~= 'sgm' then
            -- remove the clone!
           vol = vol:transpose(2, 3):transpose(3, 4)
           collectgarbage()
           do
