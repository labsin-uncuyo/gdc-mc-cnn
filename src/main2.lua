...
local function main()
   ...
   local checkpoint, optim_state = model9:load(opt)
   print('===> Loaded! Network: ' .. model9.name)
   ...
   if opt.a == 'train' then
      ...
      local trainingExpert = TrainingExpert(model9, optim_state, opt)
   ...
   if opt.a == 'test' then
      ...
      local testingExpert = TestingExpert(dataset, model9, nil, opt)
